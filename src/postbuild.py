# -*- coding: utf-8 -*-
"""Post-build patches. Run AFTER any build (buildfinal3.py) from the repo root:
   python3 src/postbuild.py
Applies: masthead brandwrap fix, homepage network-link eyebrow, editorial-team
author identifiers, shortened SEO titles/descriptions, asset version bump.
These live here (not in the data modules) because the shell mount serves stale
reads of edited modules; keeping them as an idempotent post-pass avoids regressions.
"""
import os, re, glob
...
def main():
    ...
    print("post-build patches applied to", len(files), "pages; asset version v" + ASSET_VER)
