# -*- coding: utf-8 -*-
"""shrinkiatry engine + pillar framework (clean write-once module)."""
import json

SITE = {
    "url": "https://shrinkiatry.com", "brand": "shrinkiatry", "tagline": "The profession. Understood.",
    "email": "support@shrinkiatry.com", "publisher": "shrinkMD Publishing, LLC",
    "reviewer": "Shariq Refai, MD, MBA", "year": "2026", "og_image": "https://shrinkiatry.com/opengraph.jpg",
}

NAV = [("Careers", "/careers/"), ("Business", "/business/"), ("Technology", "/technology/"),
       ("Economics", "/economics/"), ("Culture", "/culture/"), ("Innovation", "/innovation/"),
...
NETWORK_LINKS = [("About the network", "https://shrinknetwork.com/"),
                 ("Network editorial standards", "https://shrinknetwork.com/editorial-standards/"),
                 ("Crisis and safety", "https://shrinknetwork.com/crisis/"), ("Contact", "https://shrinknetwork.com/contact/")]
...
def sources_block(items):
    ...
    for text, url in items:
        lis.append(f'<li>{text} <a href="{url}"{ext(url)}>{url}</a></li>' if url else f'<li>{text}</li>')
...
def reviewer_card(date="June 27, 2026", note=None):
    ...
    return ('<aside class="reviewer" aria-label="Reviewer"><div class="reviewer__avatar" aria-hidden="true">SR</div>'
            ...
            f'<div class="reviewer__role">Board-certified psychiatrist &middot; Founder of shrinkMD &middot; Last reviewed {date}</div>'
...
def network_continue(keys, heading="Continue through The Shrink Network"):
    ...
    return f'<aside class="network-continue" aria-labelledby="nc-h"><span class="eyebrow" id="nc-h">{heading}</span><ul>' + "".join(lis) + "</ul></aside>"
...
def disclaimer_box():
    return ('<div class="disclaimer-box"><strong>Educational and professional commentary only.</strong> '
            ...
            'for evaluation or treatment by a licensed clinician, and reading it doesn\'t create a doctor-patient relationship. '
            'If you\'re looking for psychiatric care, <a href="https://shrinkmd.com" target="_blank" rel="noopener noreferrer">shrinkMD</a> is the network\'s clinical practice.</div>')

def why_trust_box():
    items = ['Written or reviewed by <a href="/editorial-team/">Shariq Refai, MD, MBA</a>, a board-certified psychiatrist',
...
def review_panel(meta):
    rows = [("Reviewer", '<a href="/editorial-team/">Shariq Refai, MD, MBA</a>, board-certified psychiatrist'),
...
def related_cluster(items, heading="Related on shrinkiatry"):
    ...
    return f'<section class="related-cluster" aria-labelledby="rel-h"><h2 id="rel-h">{heading}</h2><div class="relpills">{pills}</div></section>'
...
def article_shell(meta):
    ...
    metaline = (f'<div class="article__meta"><span>By {SITE["reviewer"]}</span><span>&middot;</span>'
                f'<span>Reviewed {meta.get("updated","June 27, 2026")}</span><span>&middot;</span>'
                f'<span>{meta.get("reading_time","8 min read")}</span></div>')
    ...
    body = (f'<div class="article__body flow">{intro}{meta["body"]}{faq}{rel}<hr>{src}'
