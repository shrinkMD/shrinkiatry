# -*- coding: utf-8 -*-
"""Homepage (fresh, mount-safe): showpiece hero, image 3, redesigned sections."""
import core as S
...
def _picks():
    return S.card_grid([
        S.card("How psychiatry residency actually works", "Four years, the PGY ladder, what each year trains, and why the first year looks like internal medicine.", "/careers/how-psychiatry-residency-works/", tag="Careers", tagclass="tag--blue"),
        S.card("Psychiatrist, psychologist, therapist: the real differences", "Different training, different tools, different scope. Why the words aren't interchangeable, and where they overlap.", "/careers/psychiatrist-vs-psychologist-vs-therapist/", tag="Careers", tagclass="tag--blue"),
        S.card("What telepsychiatry changes, and what it doesn't", "Access went up. The exam, the rules, and the clinical judgment mostly stayed the same. A clear look at both.", "/technology/what-telepsychiatry-changes/", tag="Technology", tagclass="tag--blue"),
    ], 3)
...
def _or_mini():
    ...
    for num, title, desc, href in OR_ROOMS:
        out.append(f'<a class="or-mini__card" href="{href}"><span class="or-mini__num">Room {num}</span>'
                   f'<h3>{title}</h3><p>{desc}</p><span class="arrow">Enter &rarr;</span></a>')
...
def _qcards():
    ...
    for label, href, tag in QROWS:
        ...
        out.append(f'<a class="qcard" href="{href}"{ext}><span class="tag tag--olive">{tag}</span>'
                   f'<h3>{label}</h3><span class="go">Go <span class="ar">&rarr;</span></span></a>')
...
<img src="__HERO__" width="800" height="600" alt="A psychiatrist's office at twilight: a leather chair lit by a shaft of cool window light, a desk with a warm lamp and laptop, and a wall of books." loading="eager" onerror="this.style.display='none'">
</figure>
__MEDALLION__
</div>
</div></div>
...
BODY = (BODY.replace("__DOTS__", DOTS).replace("__HERO__", HERO_IMG)
        .replace("__SPARK__", SPARK).replace("__MEDALLION__", MEDALLION)
        .replace("__PICKS__", _picks()).replace("__ORMINI__", _or_mini())
        .replace("__SECTIONS__", _sections()).replace("__QCARDS__", _qcards())
...
PAGES = [{
    "slug": "", "title": "shrinkiatry | The Profession. Understood.",
    ...
    "active": "/", "og_type": "website", "body": BODY,
}]
